    ...
    public static void main(String arg[]) {
        ...
        //    System.out.println("The smallest entry in the 3D matrix is " +
            ...
            public static int[][][] buildMat3d() {
                    ...
                    int x = 0;//counter var for slab iteration
                    //prints out slab 1
                    for (int row = 0; row < 3; row++) { //loops through the rows in slab n

                        for (int column = 0; column < matrix1[row].length; column++) {    //loops through the columns in slab n, row n

                            //assigns the respective slab,row,column to a random value in range of [1-99]
                            matrix1[x][row][column] = (int)((Math.random() * 99) + 1); 
                            
                            System.out.print(matrix1[x][row][column] + " "); //test
                            x++;    //increments x to increment to next slab
                        } //end inner for
